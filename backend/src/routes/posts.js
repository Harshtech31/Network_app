...
router.post('/:postId/save', authenticateToken, async (req, res) => {
  try {
    ...
    const post = await getItem(POSTS_TABLE, { id: postId });
    if (!post) {
      return res.status(404).json({ error: 'Post not found' });
    }
    ...
    const existingSave = await getItem('networkx-saved-posts', { id: savedPostId });
    ...
    await putItem('networkx-saved-posts', savedPost);
    
    // Invalidate cache
    await deleteCache(`saved-posts:${userId}`);
...
// DELETE /api/posts/:postId/save - Unsave/unbookmark a post
router.delete('/:postId/save', authenticateToken, async (req, res) => {
  try {
    ...
    await deleteItem('networkx-saved-posts', { id: savedPostId });
...
router.get('/saved', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { page = 1, limit = 20 } = req.query;
    const cacheKey = `saved-posts:${userId}:${page}`;
    
    // Try cache first
    const cachedSavedPosts = await getCache(cacheKey);
    if (cachedSavedPosts) {
      return res.json(cachedSavedPosts);
    }

    // Get saved posts
    const savedPosts = await queryItems(
      'networkx-saved-posts',
      'userId = :userId',
      { ':userId': userId },
      'UserSavedPostsIndex',
      parseInt(limit)
    ...
    // Get actual post details
    const postsWithDetails = await Promise.all(
      savedPosts.map(async (savedPost) => {
        const post = await getItem(POSTS_TABLE, { id: savedPost.postId });
        if (post) {
          const user = await User.findById(post.userId).select('firstName lastName username profileImage');
          return {
            ...post,
